...
const createBooking = asyncHandler(async (req, res) => {
  ...
  if (overLappingBooking) {
    throw new ApiError(
      400,
      `This item is already booked from ${overLappingBooking.startDate.toDateString()} to ${overLappingBooking.endDate.toDateString()}`,
    );
  }

  const diffDays = Math.ceil(Math.abs(end - start) / (1000 * 60 * 60 * 24));
  const totalPrice =
    diffDays * item.rentalPricePerDay + (item.depositAmount || 0);
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        requests,
        requests.length === 0
          ? "You have no incoming rental requests"
