...
export default function ItemDetail() {
  ...
  const isOwner = user && item && user._id === (item.ownerId?._id || item.ownerId);

  // Calculate rental info
  const diffDays = startDate && endDate
    ? Math.ceil((new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24))
    : 0;
  const rentalCost = diffDays > 0 ? diffDays * (item?.rentalPricePerDay || 0) : 0;
  ...
  const handleBooking = async () => {
    if (!user) { toast.error("Please login to book an item"); navigate("/login"); return; }
    if (!startDate || !endDate) { toast.error("Please select rental dates"); return; }
    if (diffDays <= 0) { toast.error("End date must be after start date"); return; }
    ...
    try {
      ...
      if (paymentMethod === "cash") {
        ...
        setBookingSuccess(res.data.message || "Booking created! Awaiting owner approval.");
        return;
      }
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            setBookingSuccess(verifyRes.data.message || "Payment successful! Booking created.");
          } catch {
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-6 py-8 w-full animate-in fade-in duration-500">
      <nav className="flex items-center space-x-2 text-slate-400 text-sm mb-8 font-medium">
        <Link to="/browse" className="hover:text-indigo-500 transition-colors">Browse</Link>
        <ChevronRight className="w-4 h-4" />
        <Link to="/browse" className="hover:text-indigo-500 transition-colors">{item.category}</Link>
        <ChevronRight className="w-4 h-4" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-7 space-y-12 animate-in slide-in-from-left-8 fade-in duration-500 delay-100">
              ...
              {!isOwner && (
                <button 
                  onClick={async () => {
                    ...
                    } catch { toast.error("Error toggling wishlist"); }
                  }}
          ...
          {/* Description */}
          <section>
            <h2 className="font-heading text-2xl font-bold text-slate-900 mb-4">Item Description</h2>
            <p className="font-sans text-slate-600 leading-relaxed">{item.description}</p>
          </section>

          {/* Rental Rules */}
          <section>
            <h2 className="font-heading text-2xl font-bold text-slate-900 mb-6">Rental Rules</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-start space-x-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                ...
                <div>
                  <h4 className="font-bold text-sm text-slate-900">Handle with care</h4>
                  <p className="text-xs text-slate-500 mt-1">Please keep the item secure when not in use.</p>
                </div>
              </div>
              <div className="flex items-start space-x-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                <ShieldCheck className="w-6 h-6 text-indigo-500 flex-shrink-0" />
                <div>
                  <h4 className="font-bold text-sm text-slate-900">Return Condition</h4>
                  <p className="text-xs text-slate-500 mt-1">Please return the item in the condition it was given.</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="sticky top-24 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <span className="px-3 py-1 bg-indigo-50 text-indigo-500 text-xs font-bold rounded-full uppercase tracking-wider">{item.category}</span>
                <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-xs font-bold rounded-full uppercase tracking-wider">{item.condition}</span>
              </div>
              <h1 className="font-heading text-3xl font-extrabold text-slate-900 leading-tight">{item.title}</h1>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="flex items-center space-x-1">
                      <p className="font-bold text-sm text-slate-900">{item.ownerId?.fullname || "Student"}</p>
                      <ShieldCheck className="w-4 h-4 text-emerald-500" />
                    </div>
                    <p className="text-xs text-emerald-600 font-bold">Verified Student</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl shadow-indigo-600/5 border border-indigo-100/50 space-y-6">
              <div className="flex justify-between items-end">
                <div>
                  <span className="text-4xl font-extrabold text-emerald-600 font-heading">₹{item.rentalPricePerDay}</span>
                  <span className="text-slate-500 font-bold"> / day</span>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Refundable Deposit</p>
                  <p className="text-lg font-bold text-emerald-500">₹{deposit}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {!isOwner && (
                  <div className="space-y-2">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Payment Method</label>
              ...
              {diffDays > 0 && (
                <div className="space-y-3 pt-4 border-t border-slate-100 font-medium">
                  <div className="flex justify-between text-sm text-slate-600">
                    <span>₹{item.rentalPricePerDay} × {diffDays} days</span>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex justify-between">
                    ...
                    <span className="font-extrabold text-emerald-500 text-lg">₹{totalPrice}</span>
                  </div>
                </div>
              )}
              ...
              {!isOwner ? (
                <div className="flex flex-col gap-3">
                  <Button
                    ...
                    disabled={isBooking || !startDate || !endDate || diffDays <= 0}
                  ...
                  >
                    {isBooking ? "Processing..." : paymentMethod === "cash" ? "Book Now (Cash)" : "Pay & Book Now"}
                  </Button>
                  <Link to={`/messages?user=${item.ownerId?._id}`} className="w-full">
                    <Button
                      variant="outline"
                      size="lg"
                ...
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-center">
                  <p className="text-sm font-bold text-amber-700">This is your listing</p>
                  <p className="text-xs text-amber-600 mt-1">You cannot rent or message yourself.</p>
                </div>
              )}
            ...
            <div className="p-6 bg-emerald-50 rounded-3xl border border-emerald-100 flex items-center space-x-4">
              ...
              <div>
                <h4 className="text-sm font-bold text-emerald-900">Student Protection</h4>
                <p className="text-xs text-emerald-700 font-medium mt-0.5">
                  All rentals are insured up to ₹10,000 for verified campus members.
                </p>
              </div>
            </div>
          </div>
